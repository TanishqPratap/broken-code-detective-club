...
const MobilePostFeed = () => {
  ...
  const fetchFeedContent = async () => {
    try {
      console.log('=== STARTING MOBILE FEED FETCH ===');
      ...
      console.error('Error fetching mobile feed content:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="space-y-0">
        {isMobile && <MobileStoriesCarousel />}
